...
const CharacterDetail = () => {
  ...
  const { data: location, isLoading: locationLoading } =
    useSingleLocationQuery(id);
  const allCharacters = location && location.residents;
  const [charactersData, setCharactersData] = useState<CharacterItem[]>([]);
  let otherCharacters = charactersData?.filter(
    (item: CharacterItem) =>
      item.location?.url.split(
        "https://rickandmortyapi.com/api/location/"
      )[1] === id && item.status === data?.status
  );
  otherCharacters = otherCharacters.filter(
    (item: CharacterItem) => String(item.id) != characterId
  );
  ...
  const slicedData = otherCharacters.slice(
      ...
      <div className={`${style.otherCharacterContainer}`}>
        ...
        <div className={`${style.allCharactersContainer}`}>
          {slicedData?.map((item: CharacterItem) => {
            return <DetailCharacterCard item={item} key={item.id}/>;
          })}
        </div>
        {totalPages > 0 && (
          <Pagination
